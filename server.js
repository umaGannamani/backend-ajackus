...
const initializeDBandServer = async () => {
    try {
        ...
        module.exports = server; // Export the server here
    } catch (e) {
...
app.get('/users/', async (request, response) => {
    ...
    response.send(
        userArray.map((userDetails) =>
            convertObjKeyPascalToCamel(userDetails)
        )
    );
});
...
app.post('/user/', async (request, response) => {
    ...
    const addUserQuery = `
    INSERT INTO 
    user (user_firstname, user_lastname, user_email, user_department)
    VALUES ('${userFirstname}', '${userLastname}', '${userEmail}', '${userDepartment}');`;
